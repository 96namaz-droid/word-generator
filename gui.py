...
class MainApplication(tk.Tk):
    ...
    def _create_roof_section(self):
        ...
        entries = [
            ...
            ("Тип ограждения", "fence_type"),
            ("Тип кровли", "roof_type"),
            ("Материал стоек", "material"),
            ("Покрытие", "coating"),
            ("Шаг крепления (м)", "mount_pitch"),
            ("Расчетная снеговая нагрузка (кПа)", "snow_load"),
            ("Замечания", "remarks"),
        ]
        frame.columnconfigure(1, weight=1)
        ...
        defaults = {
            "fence_type": "трубчатое",
            "roof_type": "эксплуатируемая кровля",
    ...
    def _create_visual_inspection_section(self):
        ...
        ttk.Label(mount_frame, text="Следы нарушения крепления к стене:").pack(side='left', padx=(0, 10))
...
• Тип ограждения: {data.get('fence_type', '')}
• Тип кровли: {data.get('roof_type', '')}
• Материал стоек: {data.get('material', '')}
• Покрытие: {data.get('coating', '')}
• Шаг крепления: {data.get('mount_pitch', '')} м
• Расчетная снеговая нагрузка: {data.get('snow_load', '')} кПа
• Замечания: {data.get('remarks', 'не указаны')}
"""
