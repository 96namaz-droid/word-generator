...
class RoofFenceGenerator(BaseProtocolGenerator):
    ...
    def _add_overview(self) -> None:
        ...
        roof_type = self.data.get('roof_type', 'эксплуатируемая кровля')
        self._add_key_value('Тип кровли', roof_type)
    ...
    def _add_geometry_section(self) -> None:
        ...
        rows = [
            ('Тип ограждения', self.data.get('fence_type')),
            ...
            ('Материал стоек', self.data.get('material', 'сталь оцинкованная')),
            ('Покрытие', self.data.get('coating', 'порошковое')),
        ]
    ...
    def _add_mounting_section(self) -> None:
        ...
        snow_load = self.data.get('snow_load', '3.2 кПа')
        text = (
            ...
            f"Расчетная снеговая нагрузка района строительства — {snow_load}."
        )
    ...
    def _add_conclusion(self) -> None:
        ...
        remarks = self.data.get('remarks', '').strip()
        if remarks:
            text = (
                "По результатам испытаний выявлены замечания: "
                f"{remarks}. Требуется устранить дефекты и повторить контроль."
            )
        else:
            text = (
